...
// GET /api/target
// ─────────────────────────────────────────────────────
export const getTarget = async (_req, res) => {
    try {
        ...
        const progressPercent = stats.targetGoal > 0
...
export const updateTarget = async (req, res) => {
    try {
        ...
        await stats.save();
        const remaining = Math.max(0, stats.targetGoal - stats.totalFunds);
        const progressPercent = Math.min(100, Math.round((stats.totalFunds / stats.targetGoal) * 100));
        res.status(200).json({
...
// ─────────────────────────────────────────────────────
export const downloadHistoryCSV = async (_req, res) => {
    try {
        ...
        res.setHeader("Content-Disposition", `attachment; filename="target-history-${Date.now()}.csv"`);
